...
kubectl get csinode $CLUSTER_NAME-worker -o yaml | grep -A 10 -B 5 ownerReferences

# Store the original CSINode UID for comparison
ORIGINAL_OWNERREF_UID=$(kubectl get csinode $CLUSTER_NAME-worker -o jsonpath='{.metadata.ownerReferences[0].uid}')
ORIGINAL_NODE_UID=$(kubectl get node $CLUSTER_NAME-worker -o jsonpath='{.metadata.uid}')
...
kubectl drain $CLUSTER_NAME-worker --ignore-daemonsets --delete-emptydir-data --force

echo "   b. Deleting worker node from cluster..."
kubectl delete node $CLUSTER_NAME-worker
...
docker restart $CLUSTER_NAME-worker

echo "   e. Waiting for node to rejoin cluster..."
wait_for_condition "kubectl get node $CLUSTER_NAME-worker --no-headers 2>/dev/null | grep -q Ready" 60 5
...
NEW_OWNERREF_UID=$(kubectl get csinode $CLUSTER_NAME-worker -o jsonpath='{.metadata.ownerReferences[0].uid}' 2>/dev/null || echo "NOT_FOUND")
NEW_NODE_UID=$(kubectl get node $CLUSTER_NAME-worker -o jsonpath='{.metadata.uid}' 2>/dev/null || echo "NOT_FOUND")
CSINODE_DRIVERS=$(kubectl get csinode $CLUSTER_NAME-worker -o jsonpath='{.spec.drivers[*].name}' 2>/dev/null || echo "NONE")
...
if [ "$NEW_OWNERREF_UID" != "$NEW_NODE_UID" ]; then
    echo "✓ Owner references don't match (race condition should occur)"
else
    echo "✗ CSINode was recreated (owner references match)"
    exit 1
fi

# Wait for CSI plugin pod to be ready
echo "10. Checking CSI plugin pod status..."
wait_for_condition "kubectl get pods csi-hostpathplugin-0 --no-headers 2>/dev/null | grep -q '8/8.*Running'" 120 10 || echo "CSI plugin pod is not ready"

# Wait for garbage collection to complete (1 minute delay)
echo "11. Waiting 1 minute for garbage collection to complete..."
echo "      (The modified garbage collector has a 1-minute delay before deleting resources)"
sleep 60

echo "      Checking if CSINode object has been garbage collected..."
if kubectl get csinode $CLUSTER_NAME-worker &>/dev/null; then
    echo "✗ CSINode still exists after garbage collection delay"
    exit 1
else
    echo "✓ CSINode has been garbage collected"
fi

echo "12. Now the CSINode object won't ever be recreated, despite the driver pod being present on the host."
echo "It's possible to run "kubectl delete po csi-hostpathplugin-0" to recreate the pod and force a new reconciliation."
