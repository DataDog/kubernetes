...
func (ss *scaleSet) getVMSSVMCache(resourceGroup, vmssName string) (string, *azcache.TimedCache, error) {
	cacheKey := fmt.Sprintf("k8svmssvm%s%s", resourceGroup, vmssName)
...
func (ss *scaleSet) deleteCacheForNode(nodeName string) error {
	klog.Errorf("BENXXX TODO deleteCacheForNode call - not implemented yet, nodeName=%s", nodeName)
	return fmt.Errorf("deleteCacheForNode not yet implemented for sharded vmss caches (node: %s)", nodeName)
	/*
