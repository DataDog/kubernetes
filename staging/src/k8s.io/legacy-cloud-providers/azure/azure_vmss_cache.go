...
// getVMSSVMCache returns an *azcache.TimedCache and that cache's key for a VMSS, creating that cache if necessary.
...
func (ss *scaleSet) gcVMSSVMCache() error {
	...
	for key := range removed {
		klog.Errorf("BENXXX ss.gcVMSSVMCache will remove cache for '%s' vmss", key)
