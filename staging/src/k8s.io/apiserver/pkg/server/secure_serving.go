...
func (s *SecureServingInfo) Serve(handler http.Handler, shutdownTimeout time.Duration, stopCh <-chan struct{}) error {
	...
	secureServer.TLSConfig.GetCertificate = s.GetCertificate
...
func (s *SecureServingInfo) RenewCert(certSource *CertInfo) (cert *tls.Certificate, err error) {
	// Autogenerated cert.
	if *certSource.KeyFile == "" {
		return s.Cert, nil
	}

	tlsCert, err := tls.LoadX509KeyPair(*certSource.CertFile, *certSource.KeyFile)
	if err != nil {
		return nil, fmt.Errorf("unable to load server certificate: %v", err)
	}
	// certSource.LoadedSourceHash = certSource.FSSourceHash
	// s.Cert = &tlsCert
	return &tlsCert, nil
}

func (s *SecureServingInfo) GetCertificate(clientHello *tls.ClientHelloInfo) (cert *tls.Certificate, err error) {
	if s.Cert == nil {
		return nil, errors.New("tls: no certificates configured")
	}

	if s.Cert != nil || len(s.SNICerts) < 1 {
		// Checking if the file has changed.
		// if s.CertSource.LoadedSourceHash != s.CertSource.FSSourceHash {
		return s.RenewCert(s.CertSource)
		// }
		// return s.Cert, nil
	}
	...
	if cert, ok := s.SNICerts[name]; ok {
		// Checking if the file has changed.
		// if s.SNICertsSources[name].LoadedSourceHash != s.SNICertsSources[name].FSSourceHash {
		cert, err = s.RenewCert(s.SNICertsSources[name])
		// }
		return cert, nil
	}
...
// getNamedCertificateMap returns a map of *CertInfo by name.
// Returns nil if len(certs) == 0
func GetNamedCertificateMapSources(certs []NamedTLSCert) (map[string]*CertInfo, error) {
	// register certs with implicit names first, reverse order such that earlier trump over the later
