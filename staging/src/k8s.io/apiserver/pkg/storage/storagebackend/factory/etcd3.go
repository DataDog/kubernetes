...
var newETCD3Client = func(c storagebackend.TransportConfig) (*kubernetes.Client, error) {
  ...
  "load_balancing_config": [{
	...
	if len(c.ServerList) > 1 {
		...
		// Strip the scheme (https://) from each endpoint since gRPC handles TLS separately
